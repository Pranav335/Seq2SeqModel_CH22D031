# -*- coding: utf-8 -*-
"""
Created on Fri May 19 22:20:46 2023
...
class Attention(nn.Module):
    ...
    def __init__(self,embedding_size,
        ...
        self.attention = nn.Linear(hidden_size * (encoder.D + 1), 1)
        self.relu = nn.ReLU()
        self.att_softmax = nn.Softmax(dim = 0)
        
        if cell_type == 'GRU':
            self.gru = nn.GRU(hidden_size * encoder.D + embedding_size, hidden_size, num_layers,
                              dropout = dropout, bidirectional =  bool(bidirectional))
        elif cell_type == 'RNN':
            self.rnn = nn.RNN(hidden_size * encoder.D + embedding_size, hidden_size, num_layers,
                              dropout = dropout, bidirectional =  bool(bidirectional))
        elif cell_type == 'LSTM':
            self.lstm = nn.LSTM(hidden_size * encoder.D + embedding_size, hidden_size, num_layers,
                              dropout = dropout, bidirectional =  bool(bidirectional))
        else:
            raise ValueError('Invalid cell type argument')
            
        self.linear = nn.Linear(self.D * hidden_size, output_size)
        self.output_softmax = nn.LogSoftmax(dim = 2)
        
        
        
    def forward(self, input_vector, encoder_output, decoder_hidden):
        
        
        sequence_length = encoder_output.shape[0]
        # Extracting the final hidden layer of the previous layer.
        if self.cell_type == 'LSTM':
            decoder_hidden1, _ = decoder_hidden
            h_repeat = decoder_hidden1[-1].repeat(sequence_length, 1, 1)
        else:
            h_repeat = decoder_hidden[-1].repeat(sequence_length, 1, 1)
            
        # Concatenating the previous decoder hidden state with encoder hidden state.
        att_input = torch.cat((h_repeat, encoder_output), dim = 2)
        energy = self.attention(att_input)
        self.att_wts = self.att_softmax(self.relu(energy))
        self.att_wts = self.att_wts.permute(1, 2, 0)
        encoder_states = encoder_output.permute(1, 0, 2)
        context_vector = torch.bmm(self.att_wts, encoder_states)
        # Context vector
        context_vector = context_vector.permute(1, 0, 2)
        
        embedding = self.embedding(input_vector).view(1, self.batch_size, -1)
        # cell input
        gru_input = torch.cat((context_vector, embedding), dim = 2)
        
        if self.cell_type != "LSTM":
            decoder_hidden = decoder_hidden[-self.D:].repeat(self.num_layers, 1, 1)
        
        if self.cell_type == 'GRU':
            decoder_output, decoder_hidden = self.gru(gru_input, decoder_hidden)
        elif self.cell_type == 'RNN':
            decoder_output, decoder_hidden = self.rnn(gru_input, decoder_hidden)
        else:
            ho, co = decoder_hidden
            decoder_output, decoder_hidden = self.lstm(gru_input, (ho,co))
        
        
        # Fully connected layer
        decoder_output = self.output_softmax(self.linear(decoder_output))
...
def trainiter(encoder, decoder, dataloader,
    ...
    for j in range(epochs):
        # Implementated the teacher forcing of 0.5
...
if __name__ == "__main__":
    ...
    if args.sweep:
        ...
        decoder = Attention(embedding_size = embedding_size,
                              hidden_size = hidden_size, 
        ...
        torch.save(encoder.state_dict(), './encoder_attention.py')
        torch.save(decoder.state_dict(), './decoder_attention.py')
    else:
        ...
        def wandbsweep():
            ...
            decoder = Attention(embedding_size = wandb.config.embedding_size,
                                  hidden_size = wandb.config.hidden_size, 
                                  output_size = output_size,
                                  num_layers = wandb.config.num_layer,
                                  batch_size = batch_size,
